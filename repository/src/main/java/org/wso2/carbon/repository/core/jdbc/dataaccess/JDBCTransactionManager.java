...
public class JDBCTransactionManager implements TransactionManager {
    ...
    public void beginTransaction() throws RepositoryException {
        if (dataAccessManager.getDatabaseTransaction().getNestedDepth() != 0) {
            if (log.isTraceEnabled()) {
                log.trace("The transaction was not started, because it is called within a transaction, nested depth: " +
    ...
    public void rollbackTransaction() throws RepositoryException {
        ...
        if (dataAccessManager.getDatabaseTransaction().getNestedDepth() != 1) {
            if (log.isTraceEnabled()) {
                log.trace("The transaction was not rollbacked, because it is called within a transaction, nested depth: " +
                                dataAccessManager.getDatabaseTransaction().getNestedDepth() + ".");
            }
    ...
    public void commitTransaction() throws RepositoryException {
        if (dataAccessManager.getDatabaseTransaction().getNestedDepth() != 1) {
            if (log.isTraceEnabled()) {
                log.trace("The transaction was not committed, because it is called within a transaction, nested depth: " +
                                dataAccessManager.getDatabaseTransaction().getNestedDepth() + ".");
            }
        ...
        if (dataAccessManager.getDatabaseTransaction().isRollbacked()) {
            String msg = "The transaction is already rollbacked, you can not commit a transaction already rollbacked, nested depth: " +
                            dataAccessManager.getDatabaseTransaction().getNestedDepth() + ".";
    ...
    private void endTransaction() throws RepositoryException {
