...
import org.wso2.carbon.repository.core.CollectionVersionImpl;
...
public class JDBCResourceVersionDAO implements ResourceVersionDAO {
    ...
    public void fillResourceContentArchived(ResourceImpl resourceImpl) throws RepositoryException {
        JDBCDatabaseTransaction.ManagedRegistryConnection conn = JDBCDatabaseTransaction.getConnection();
        ...
        try {
            ...
            if (result1.next()) {
                resourceImpl.setContentStreamWithNoUpdate(
                        RepositoryUtils.getMemoryStream(
    ...
    public ResourceImpl get(ResourceIDImpl resourceID, long snapshotID) throws RepositoryException {
        ...
        if (resourceID.isCollection()) {
            ...
            if (tempTenantId != MultitenantConstants.SUPER_TENANT_ID && tempTenantId != MultitenantConstants.INVALID_TENANT_ID) {
                String[] childPaths = getChildPaths(resourceID, versionRetriever, versionIndex,
    ...
    public boolean resourceExists(ResourceIDImpl resourceID, long snapshotID) throws RepositoryException {
        VersionRetriever versionRetriever = getVersionList(snapshotID);
        ...
        while (true) {
            ...
            resourceDO = getResourceDOArchived(version);
            if (resourceDO.getPathID() == resourceID.getPathID() &&
                    ((resourceID.isCollection() && resourceDO.getName() == null) || (resourceID.getName() != null &&
                                    resourceID.getName().equals(resourceDO.getName())))) {
                break;
            }
            resourceDO = null;
    ...
    public VersionRetriever getVersionList(ResourceIDImpl resourceID, long snapshotID) throws RepositoryException {
        JDBCDatabaseTransaction.ManagedRegistryConnection conn = JDBCDatabaseTransaction.getConnection();
        ...
        try {
            ...
            if (result.next()) {
                InputStream resourceVIDStream = RepositoryUtils.getMemoryStream(
    ...
    public CollectionImpl get(ResourceIDImpl resourceID, long snapshotID,
                              int start, int pageLen) throws RepositoryException {
        if (!resourceID.isCollection()) {
        ...
        while (true) {
            ...
            resourceDO = getResourceDOArchived(version);
            if (resourceDO.getPathID() == resourceID.getPathID() && ((resourceID.isCollection() && resourceDO.getName() == null) ||
                            (resourceID.getName() != null &&
                                    resourceID.getName().equals(resourceDO.getName())))) {
                break;
            }
    ...
    public void fillChildren(CollectionImpl collectionImpl, VersionRetriever versionRetriever,
                             int parentVersionIndex, int start, int pageLen, long snapshotID) throws RepositoryException {
        JDBCDatabaseTransaction.ManagedRegistryConnection conn = JDBCDatabaseTransaction.getConnection();
     ...
     */
    public int getChildCount(String resourceID, long versionNumber, Connection conn) throws RepositoryException {
        ResultSet results = null;
        PreparedStatement ps = null;
    ...
    public String[] getChildPaths(ResourceIDImpl resourceID, VersionRetriever versionRetriever,
        ...
        if (Transaction.isStarted()) {
            childPaths = getChildPaths(resourceID, versionRetriever, parentVersionIndex,
            ...
            try {
                ...
                childPaths = getChildPaths(resourceID, versionRetriever, parentVersionIndex, start, pageLen, snapshotID, conn);
     ...
     * @throws RepositoryException throws if the operation failed.
     */
    public String[] getChildPaths(ResourceIDImpl resourceID, VersionRetriever versionRetriever, int parentVersionIndex, int start, int pageLen,
                                  long snapshotID, Connection conn) throws RepositoryException {
            ...
            try {
                ...
                if (result.next()) {
                    ...
                    if (pathId == resourceID.getPathID() && resourceName != null) {
                        ...
                        if (result2.next()) {
                            int parentPathId = result2.getInt(DatabaseConstants.PATH_PARENT_ID_FIELD);
                            if (parentPathId == resourceID.getPathID()) {
                                // so we confirm that this is a child of our collection
                                String childPath = result2.getString(DatabaseConstants.PATH_VALUE_FIELD) + InternalConstants.VERSION_SEPARATOR + snapshotID;
                                if (current >= start) {
                                    childPathList.add(childPath);
                                }
                                isValidPath = true;
    ...
    public boolean isResourceHistoryExist(long version) throws RepositoryException {
        JDBCDatabaseTransaction.ManagedRegistryConnection conn = JDBCDatabaseTransaction.getConnection();
        PreparedStatement ps = null;
        ResultSet result = null;
    ...
    public boolean isResourceHistoryExist(String path) throws RepositoryException {
    ...
    public boolean isResourceHistoryExist(ResourceIDImpl resourceID) throws RepositoryException {
        JDBCDatabaseTransaction.ManagedRegistryConnection conn = JDBCDatabaseTransaction.getConnection();
        ResultSet result = null;
        PreparedStatement ps = null;
        ...
        try {
            if (resourceID.isCollection()) {
                String sql = "SELECT REG_PATH_ID FROM REG_RESOURCE_HISTORY WHERE REG_PATH_ID=? AND REG_NAME IS NULL AND REG_TENANT_ID=?";
                ...
                String sql = "SELECT REG_PATH_ID FROM REG_RESOURCE_HISTORY WHERE REG_PATH_ID=? AND REG_NAME = ? AND REG_TENANT_ID=?";
    ...
    public boolean isContentHistoryExist(int contentId) throws RepositoryException {
        JDBCDatabaseTransaction.ManagedRegistryConnection conn = JDBCDatabaseTransaction.getConnection();
        ResultSet result = null;
        PreparedStatement ps = null;
        try {
    ...
    public String restoreResources(long version, long snapshotID) throws RepositoryException {
        ResourceDO resourceDO = getResourceDOArchived(version);
        String resourcePath = resourceDAO.getPath(resourceDO.getPathID(), resourceDO.getName(), false);
        ...
        if (StaticConfiguration.isVersioningProperties()) {
            ...
            if (linkRestoration != null) {
                ...
                if (parts.length == 4) {
                	...
                	InternalUtils.registerHandlerForSymbolicLinks(RepositoryContext.getBaseInstance(), parts[0], parts[1], parts[2]);
                }
            }
        }
    ...
    private ResourceDO getResourceDOArchived(long version) throws RepositoryException {
        JDBCDatabaseTransaction.ManagedRegistryConnection conn = JDBCDatabaseTransaction.getConnection();
        ResultSet result = null;
        PreparedStatement ps = null;
    ...
    public void versionResource(ResourceDO resourceDO, boolean keepProperties) throws RepositoryException {
        if (resourceDO.getName() != null) {
    ...
    public void putResourceToHistory(ResourceDO resourceDO) throws RepositoryException {
        JDBCDatabaseTransaction.ManagedRegistryConnection conn = JDBCDatabaseTransaction.getConnection();
        PreparedStatement ps = null;
        ...
        try {
            ...
            ps.setInt(11, CurrentContext.getTenantId());
    ...
    public void removeSnapshot(long snapshotId) throws RepositoryException {
        JDBCDatabaseTransaction.ManagedRegistryConnection conn = JDBCDatabaseTransaction.getConnection();
        PreparedStatement ps = null;
        try {
    ...
    public void removePropertyValues(long regVersionId) throws RepositoryException {
    	JDBCDatabaseTransaction.ManagedRegistryConnection conn = JDBCDatabaseTransaction.getConnection();
        PreparedStatement ps1 = null;
        PreparedStatement ps2 = null;
    ...
    private List<Long> getPropertyIds(long regVersionId) throws RepositoryException {
        JDBCDatabaseTransaction.ManagedRegistryConnection conn = JDBCDatabaseTransaction.getConnection();
        PreparedStatement ps = null;
        ResultSet results = null;
