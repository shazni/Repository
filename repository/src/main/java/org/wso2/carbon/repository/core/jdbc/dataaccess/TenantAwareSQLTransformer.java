...
public class TenantAwareSQLTransformer {
    ...
    private void sanityCheckSQL(String sqlQuery) throws RepositoryException{
        if(sqlQuery.trim().length() == 0){
            throw new RepositoryDBException("SQL String is empty");
        }
        ...
        if (! sqlQueryUCase.startsWith(G_SQL_START)){
            throw new RepositoryDBException("SQL query does not start with "+G_SQL_START);
        }
        ...
        for (String s : new String[] {MS_COMMENT_START, MS_COMMENT_END, MS_COMMENT1_START,
            ...
            int index = sqlQueryUCase.indexOf(s);
            if (index > 0 ){
                throw new RepositoryDBException("SQL query contains "+s);
            }
        }
    }
