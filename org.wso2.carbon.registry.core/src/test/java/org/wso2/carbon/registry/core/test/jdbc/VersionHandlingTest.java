...
//            embeddedRegistryService = ctx.getEmbeddedRegistryService();
    ...
    public void testResourceContentVersioning() throws RepositoryException {
        ...
        assertEquals("r1's first version's content should be 'content 1'",
                RepositoryUtils.decodeBytes((byte[]) r1vv1.getContent()), "content 1");

        Resource r1vv2 = registry.get(r1Versions[0]);

        assertEquals("r1's second version's content should be 'content 2'",
                RepositoryUtils.decodeBytes((byte[]) r1vv2.getContent()), "content 2");
    }
    ...
    public void testResourceRestore() throws RepositoryException {
        ...
        assertEquals("Restored resource should have content 'content 1'",
                "content 1", RepositoryUtils.decodeBytes((byte[]) r1r1.getContent()));
    }
    ...
    public void testRootLevelVersioning() throws RepositoryException {
        ...
        assertTrue("Root should have child vtr1",
                RepositoryUtils.containsAsSubString("/vtr1", rootV0Children));
        assertTrue("Root should have child vtc2",
                RepositoryUtils.containsAsSubString("/vtc2", rootV0Children));

        Collection rootV1 = (Collection) registry.get(rootVersions[1]);
        String[] rootV1Children = (String[]) rootV1.getContent();
        assertTrue("Root should have child vtr1",
                RepositoryUtils.containsAsSubString("/vtr1", rootV1Children));
        assertFalse("Root should not have child vtc2",
                RepositoryUtils.containsAsSubString("/vtc2", rootV1Children));
    }
}
