...
import org.wso2.carbon.registry.core.app.APPConstants;
import org.wso2.carbon.registry.core.app.Properties;
import org.wso2.carbon.registry.core.app.Property;
import org.wso2.carbon.registry.core.app.PropertyExtensionFactory;
import org.wso2.carbon.registry.core.app.PropertyName;
import org.wso2.carbon.registry.core.app.PropertyValue;
...
/**
     ...
     */
    public RemoteRegistry(URL registryURL, RegistryService registryService) {
     ...
     */
    public RemoteRegistry(URL registryURL, String userName, String password, RegistryService registryService)
     ...
     */
    public RemoteRegistry(String registryURL, String userName, String password, RegistryService registryService)
            throws MalformedURLException, RepositoryException {
        this(new URL(registryURL), userName, password, registryService);
    }
    ...
    public Resource get(String path) throws RepositoryException {
        ...
        if (path.endsWith(RepositoryConstants.URL_SEPARATOR + APPConstants.PARAMETER_COMMENTS)) {
            encodedPath = encodeURL(path);
            if (encodedPath.contains(RepositoryConstants.VERSION_SEPARATOR)) {
                int index = encodedPath.lastIndexOf(RepositoryConstants.VERSION_SEPARATOR);
        ...
        if (clientResponse.getType() == Response.ResponseType.CLIENT_ERROR ||
            ...
            if (clientResponse.getStatus() == HttpURLConnection.HTTP_NOT_FOUND) {
                ...
                throw new RepositoryUserContentException(path, RepositoryErrorCodes.HTTP_NOT_FOUND);
            }
            abderaClient.teardown();
            throw new RepositoryServerException(clientResponse.getStatusText());
        }
        ...
        if (introspection instanceof Feed) {
            ...
            if (state != null && state.equals("Deleted")) {
                abderaClient.teardown();
                throw new RepositoryResourceNotFoundException(path, RepositoryErrorCodes.RESOURCE_UNAVAILABLE);
            }
   ...
   * size configured in registry.xml */
        if (!cache.cacheResource(path, resource, eTag,
                RepositoryConstants.MAX_REG_CLIENT_CACHE_SIZE)) {
...
//    }
    ...
    public Collection get(String path, int start, int pageSize) throws RepositoryException {
        ...
        if (clientResponse.getType() == Response.ResponseType.CLIENT_ERROR ||
            ...
            if (clientResponse.getStatus() == HttpURLConnection.HTTP_NOT_FOUND) {
                abderaClient.teardown();
                throw new RepositoryResourceNotFoundException(path);
            }
            abderaClient.teardown();
            throw new RepositoryException(clientResponse.getStatusText());
        }
        Element introspection = clientResponse.getDocument().getRoot();
        if (!(introspection instanceof Feed)) {
            abderaClient.teardown();
            throw new RepositoryException("Got " + introspection.getQName() +
                    " when expecting <feed>!");
        }
    ...
    private CollectionImpl createResourceFromFeed(Feed feed)
        ...
        if (state != null && "Deleted".equals(state)) {
            resource.setState(RepositoryConstants.DELETED_STATE);
        }
    ...
    private ResourceImpl createResourceFromEntry(Entry entry) throws RepositoryException {
        ...
        org.wso2.carbon.registry.core.app.Properties properties =
        ...
        if (srcIri != null) {
            try {
                final URL url = new URL(Utils.encodeRegistryPath(
                        URLDecoder.decode(getFilteredTenantCountedContentURL(srcIri.toString()), RepositoryConstants.DEFAULT_CHARSET_ENCODING)) +
                        ((resource.getPermanentPath() != null) ?
                                RepositoryConstants.URL_SEPARATOR + "version:" + snapshotID : ""));
                ...
                throw new RepositoryServerContentException("unable to receive source. " + srcIri.toString());
            }
        }
...
//    }
    ...
    public String put(String suggestedPath, Resource resource) throws RepositoryException {
        ...
        if (Pattern.matches("\\p{Alnum}*[~!@#%^&*()\\+\\;<>\\[\\]{},/\\\\\"\',]+\\p{Alnum}*",
                relativePath)) {
            throw new RepositoryUserContentException(
                    "Invalid characters have been used in the resource name. " + relativePath
                            + ". Special characters are ~!@#%^*()+{}[]|\\<>;\"\',", RepositoryErrorCodes.INVALID_PATH_PROVIDED);
        }
...
//        impl.setParentPath(i == 0 ? "/" : resultPath.substring(0, i));
    ...
    public String importResource(String suggestedPath, String sourceURL, Resource resource)
        ...
        if (Pattern.matches("\\p{Alnum}*[~!@#%^&*()\\+=\\-;<>\\s\\[\\]{},/\\\\\"\',]+\\p{Alnum}*",
                relativePath)) {
            throw new RepositoryUserContentException("Invalid characters have been used in the resource name.", RepositoryErrorCodes.INVALID_CHARACTER_IN_NAME);
        }
        AbderaClient abderaClient = new AbderaClient(abdera);
        final Factory factory = abdera.getFactory();
        Entry entry = factory.newEntry();
        ...
        ClientResponse response =
                abderaClient.post(baseURI + APPConstants.ATOM + "?importURL=" +
                        encodeURL(sourceURL + RepositoryConstants.URL_SEPARATOR +
    ...
    public void createVersion(String path) throws RepositoryException {
        ...
        ClientResponse clientResponse =
                abderaClient.post(baseURI + APPConstants.ATOM +
                        encodeURL(path + RepositoryConstants.URL_SEPARATOR +
        ...
        if (status < 200 || status > 299) {
            ...
            if (status == HttpURLConnection.HTTP_NOT_FOUND) {
                e = new RepositoryResourceNotFoundException(path, RepositoryErrorCodes.HTTP_NOT_FOUND);
            } else {
                e = new RepositoryServerContentException("Response Status: " + clientResponse.getStatusText());
            }
            abderaClient.teardown();
            throw e;
        }
    ...
    public Association[] getAssociations(String resourcePath, String associationType)
        ...
        ClientResponse clientResponse =
                abderaClient.get(baseURI + APPConstants.ATOM +
                        encodeURL(resourcePath + RepositoryConstants.URL_SEPARATOR +
                                APPConstants.ASSOCIATIONS + ":" +
                                associationType),
                        getAuthorization());
        Document introspection = clientResponse.getDocument();
    ...
    public void removeTag(String path, String tag) throws RepositoryException {
        ...
        String encodedTag;
        try {
            encodedTag = URLEncoder.encode(tag, RepositoryConstants.DEFAULT_CHARSET_ENCODING);
        } catch (Exception e) {
    ...
    public String addComment(String resourcePath, org.wso2.carbon.repository.Comment comment)
        ...
        return addComment(resourcePath, (org.wso2.carbon.registry.core.CommentImpl) comment);
    }

    public String addComment(String resourcePath, org.wso2.carbon.registry.core.CommentImpl comment)
            throws RepositoryException {
        AbderaClient abderaClient = new AbderaClient(abdera);
        Entry entry = abdera.getFactory().newEntry();
    ...
    public org.wso2.carbon.registry.core.CommentImpl[] getComments(String _resourcePath)
            throws RepositoryException {
        AbderaClient abderaClient = new AbderaClient(abdera);
        String resourcePath = _resourcePath;
        resourcePath = encodeURL(resourcePath);
        if (resourcePath.contains(RepositoryConstants.VERSION_SEPARATOR)) {
            int index = resourcePath.lastIndexOf(RepositoryConstants.VERSION_SEPARATOR);
        ...
        ClientResponse clientResponse =
                abderaClient.get(baseURI + APPConstants.ATOM +
                        resourcePath + RepositoryConstants.URL_SEPARATOR +
                        APPConstants.PARAMETER_COMMENTS,
                        getAuthorization());
        Document introspection = clientResponse.getDocument();
        Element element = introspection.getRoot();
        Feed feed = (Feed) element;
        org.wso2.carbon.registry.core.CommentImpl[] comments = getCommentsFromFeed(feed);
        abderaClient.teardown();
    ...
    private org.wso2.carbon.registry.core.CommentImpl[] getCommentsFromFeed(Feed feed) {
        List entries = feed.getEntries();
        org.wso2.carbon.registry.core.CommentImpl comments[] = null;
        if (entries != null) {
            comments = new org.wso2.carbon.registry.core.CommentImpl[entries.size()];
            for (int i = 0; i < entries.size(); i++) {
                Entry entry = (Entry) entries.get(i);
                org.wso2.carbon.registry.core.CommentImpl comment =
                        new org.wso2.carbon.registry.core.CommentImpl();
    ...
    public static String buildQueryString(Map parameters) {
        ...
        for (Object keyObj : parameters.keySet()) {
            ...
            try {
                buffer.append(URLEncoder.encode(name, RepositoryConstants.DEFAULT_CHARSET_ENCODING));
                buffer.append("=");
                buffer.append(URLEncoder.encode(value, RepositoryConstants.DEFAULT_CHARSET_ENCODING));
    ...
    public static Map decodeQueryString(String _value) {
        ...
        String value=_value;
        try {
            value = URLDecoder.decode(value, RepositoryConstants.DEFAULT_CHARSET_ENCODING);
        } catch (UnsupportedEncodingException e) {
            return null;
        }
...
//            paramMap.put(paramParts[0], paramParts[1]);
    ...
    public Activity[] getLogs(String _resourcePath,
                              int action,
                              String userName,
                              Date from,
                              Date to,
        ...
        Activity logs[] = null;
        if (entries != null) {
            logs = new Activity[entries.size()];
            for (int i = 0; i < entries.size(); i++) {
                Entry entry = (Entry) entries.get(i);
...
/*
    ...
    */
    ...
    public void addUser(String userName,
                        ...
                        String roleName,
                        String friendlyName) throws RepositoryException {
     ...
     */
    public void restore(String path, Reader reader, int timeout) throws RepositoryException {
        AbderaClient abderaClient = new AbderaClient(abdera);
     ...
     * @throws RepositoryException if the operation failed.
     */
    public void dump(String path, int timeout, Writer writer) throws RepositoryException {
        AbderaClient abderaClient = new AbderaClient(abdera);
        abderaClient.setSocketTimeout(timeout);
        dump(path, abderaClient, writer);
        abderaClient.teardown();
        ...
        if (clientResponse.getType() == Response.ResponseType.SUCCESS) {
            ...
            if (element instanceof OMElement) {
                try {
                    ...
                    throw new RepositoryServerContentException("Failed to serialize the xml", e);
                }
            }
        } else {
    ...
    private String getFilteredTenantCountedContentURL(String srcIri) throws RepositoryException {
        ...
        try {
            ...
            throw new RepositoryUserContentException("Malformed Host Url " + srcIri, RepositoryErrorCodes.INVALID_OR_MALFORMED_URL);
        }
        return url;
    }
