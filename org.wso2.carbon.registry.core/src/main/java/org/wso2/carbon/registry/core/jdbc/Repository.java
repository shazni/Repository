...
public class Repository {
    ...
    private void prepareUpdate(Resource resource, ResourceIDImpl resourceID,
                               ResourceDO oldResourceDO) throws RepositoryException {
...
//        ((ResourceImpl) resource).setUUID(oldResourceDO.getUUID());
    ...
    public String rename(ResourcePath oldResourcePath, String newName) throws RepositoryException {
        ...
        if (!newName.startsWith(RepositoryConstants.PATH_SEPARATOR)) {
            // relative URL
            String oldDir =
                    oldPath.substring(0, oldPath.lastIndexOf(RepositoryConstants.PATH_SEPARATOR) + 1);
        ...
        for (ResourceIDImpl sourceChildID : sourceChildIDs) {
            if (!sourceChildID.isCollection()) {
                ...
                String targetChildPath =
                        targetPath + (targetPath.endsWith(RepositoryConstants.PATH_SEPARATOR) ? "" :
        ...
        if (sourceResource instanceof CollectionImpl) {
            ...
            for (String childSourcePath : collection.getChildren()) {
                ...
                String childTargetPath =
                        targetPath + RepositoryConstants.PATH_SEPARATOR + childResourceName;
    ...
    private void add(String path, ResourceImpl resource) throws RepositoryException {
        ...
        if (!Boolean.FALSE.equals(CurrentSession.getAttribute(IS_LOGGING_ACTIVITY))) {
            registryContext.getLogWriter().addLog(
                    path, CurrentSession.getUser(), Activity.ADD, null);
        }
    ...
    private void addEmptyCollection(String path) throws RepositoryException {
        ...
        if (parentPath != null) {
            ...
            if (parentResourceID == null || !resourceDAO.resourceExists(parentResourceID)) {
                ...
                log.error(msg);
                throw new RepositoryAuthException(msg);
            }
            ...
            // getting rating information
                ...
                while (xmlReader.hasNext() &&
                    ...
                    if (destination.startsWith(
                        ...
                        destination =
                        		RepositoryUtils.getAbsoluteAssociationPath(destination, path);
                    }
        ...
        if (linkRestoration != null) {
            ...
            if (parts.length == 4) {
                ...
                if (parts[0] != null && parts[1] != null) {
                    ...
                    String relativeTargetPath = RepositoryUtils.getRelativeAssociationPath(parts[1],
                            parts[0]);
                    // then we derive the absolute path with reference to the current path.
                    String absoluteTargetPath = RepositoryUtils.getAbsoluteAssociationPath(
...
/*                    if (!incomingChildPath.startsWith(incomingParentPath)) {
    ...
    private void dumpRecursively(String path,
        ...
        // getting comment information
        CommentImpl[] comments = commentsDAO.getComments(resource);
        ...
        if (associations != null && associations.length > 0) {
            ...
            for (Association association : associations) {
                ...
                source = RepositoryUtils.getRelativeAssociationPath(source, path);

                if (destination.startsWith(RepositoryConstants.ROOT_PATH)) {
                    // we are treating this as a path
                    destination = RepositoryUtils.getRelativeAssociationPath(destination, path);
                } else {
