...
public class ChrootWrapper {
    ...
    private void fixCollection(CollectionImpl collection) throws RepositoryException {
        ...
        if (content instanceof String[]) {
            ...
            for (Resource resource : resources) {
                ...
                if (resource instanceof CommentImpl) {
                    CommentImpl comment = (CommentImpl) resource;
                    comment.setResourcePath(getOutPath(comment.getResourcePath()));
                }
            }
        }
    ...
    public Activity[] fixLogEntries(Activity[] logEntries) {
        ...
        for (Activity logEntry : logEntries) {
            ...
            if (logPath == null ||
                    (!logPath.startsWith(basePrefix + RepositoryConstants.PATH_SEPARATOR) &&
            ...
            } else if (logEntry.getAction() == Activity.ADD_ASSOCIATION ||
                    logEntry.getAction() == Activity.REMOVE_ASSOCIATION) {
