...
//    private JdbcDirectory jdbcDir;
     ...
     */
    public static RegistryContext getBaseInstance(RealmService realmService, RegistryService registryService) {
        return getBaseInstance(realmService, true, registryService);
    }
    ...
    public static RegistryContext getBaseInstance(RealmService realmService, 
                                                  boolean populateConfiguration, RegistryService registryService) {
        try {
            // Return existing instance or create a new one.
            if (getBaseInstance() != null) {
                return getBaseInstance(); 
            } 
            new RegistryContext(realmService, populateConfiguration, registryService);
    ...
    public static RegistryContext getBaseInstance(InputStream configStream,
                                                  RealmService realmService, RegistryService registryService) {
        try {
            // Return existing instance or create a new one.
            if (getBaseInstance() != null) {
                return getBaseInstance();
            }
            new RegistryContext(configStream, realmService, registryService);
        } catch (RepositoryException e) {
            log.error("Unable to get instance of the registry context", e);
            return null;
        }
    ...
    @SuppressWarnings("deprecation")
    public static RegistryContext getBaseInstance(InputStream configStream,
                                                  RegURLSupplier urlSupplier, RegistryService registryService) {
        try {
            // Return existing instance or create a new one.
            if (getBaseInstance() != null) {
                return getBaseInstance();
            }
            new RegistryContext(configStream, urlSupplier, registryService);
        } catch (RepositoryException e) {
            log.error("Unable to get instance of the registry context", e);
            return null;
        }
     ...
     */
    protected RegistryContext(RealmService realmService, boolean populateConfiguration, RegistryService registryService)
            throws RepositoryException {
        this(null, realmService, populateConfiguration, registryService);
    }
     ...
     * @throws RepositoryException throws if the construction failed.
     */
    protected RegistryContext(InputStream configStream, RealmService realmService, RegistryService registryService)
            throws RepositoryException {
        this(configStream, realmService, true, registryService);
    }
    ...
    protected RegistryContext(InputStream configStream, RealmService realmService, 
        ...
        if (populateConfiguration) {
            RegistryConfigurationProcessor.populateRegistryConfig(configStream, this, registryService);
        }
    }
     ...
     */
    @SuppressWarnings("deprecation")
    protected RegistryContext(InputStream configStream, RegURLSupplier urlSupplier, RegistryService registryService)
            throws RepositoryException {
        // setBaseInstance will do the necessary security check.
        setBaseInstance(this);
...
/*    *//**
    ...
    *//**
     ...
     * @throws RepositoryException throws if the retrieval of the embedded registry service is
     *                           failed.
     */
    public EmbeddedRegistryService getEmbeddedRegistryService() throws RepositoryException {
