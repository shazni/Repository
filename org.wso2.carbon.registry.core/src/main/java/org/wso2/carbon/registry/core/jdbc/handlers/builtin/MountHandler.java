...
public class MountHandler extends Handler {
    ...
    @SuppressWarnings("unchecked")
    private Registry getRegistry(RequestContext requestContext) throws RepositoryException {
        ...
        if (!isHandlerRegistered && baseContext != null) {
            try {               
                ...
                if (subPath != null) {
                    InternalUtils.addMountEntry(systemRegistry, RegistryContext.getBaseInstance(),
        ...
        if (remote) {
            ...
            InternalUtils.setTrustStoreSystemProperties();
//            try {
//                if (baseContext != null) {
//                    Registry registry = new RemoteRegistry(this.conURL, this.userName, this.password, null) {
//
...
//                            baseContext.getRealmService(), baseContext.getRegistryRoot());
//                }
//                return new RemoteRegistry(this.conURL, this.userName, this.password, null);
//
//            } catch (MalformedURLException e) {
//                String msg = "Unable to connect to the remote registry";
//                log.error(msg, e);
//                throw new RepositoryUserContentException(msg, e, RepositoryErrorCodes.INVALID_OR_MALFORMED_URL);
//            }
        }
        if (registryService == null) {
            ...
            registryService.setIsClone(true);
    ...
    public void put(RequestContext requestContext) throws RepositoryException {
        ...
        try {
            ...
            if( RepositoryConstants.XSD_MEDIA_TYPE.equals(resource.getMediaType())
...
//                registryContext = ((RemoteRegistry) remoteRegistry).getRegistryContext();
            	registryContext = InternalUtils.getRegistryContext(remoteRegistry);
            } finally {
                endNestedOperation();
            }
        } catch (Exception e) {
...
//                                                requestContext.getVersionRepository()
        ...
        if (remote) {
            ...
            //tempResource.addProperty(RepositoryConstants.REGISTRY_REAL_PATH, this.conURL +
            // "/resourceContent?path=" + actualPath);
        } else {
            ((ResourceImpl) tempResource).addPropertyWithNoUpdate(RepositoryConstants.REGISTRY_USER, CurrentSession.getUser());
            //tempResource.addProperty(RepositoryConstants.REGISTRY_REAL_PATH, actualPath);
        }
        ((ResourceImpl) tempResource).addPropertyWithNoUpdate(RepositoryConstants.REGISTRY_REAL_PATH,
...
//        RegistryContext registryContext = requestContext.getRegistryContext();
        RegistryService registryService = requestContext.getRegistry().getRegistryService();
        if (fullPath.equals(this.mountPoint)) {
            requestContext.getRegistry().removeLink(requestContext.getResource().getPath());
            if (/*registryContext != null*/ registryService != null) {
//                HandlerManager hm = registryContext.getHandlerManager();
//            	HandlerManager hm = registryService.getHandlerManager();
            	registryService.removeHandler(this,
...
//            newPath = (requestContext.getRegistry()).rename(fullResourcePath, fullTargetPath);
            newPath = ((EmbeddedRegistry) (requestContext.getRegistry())).getRepository()
                    .rename(new ResourcePath(fullResourcePath), fullTargetPath);
...
//            	(requestContext.getRegistry()).move(fullResourcePath, fullTargetPath);
                ((EmbeddedRegistry) requestContext.getRegistry()).getRepository().move(new ResourcePath(fullResourcePath), fullTargetPath);
                requestContext.getRegistry().createLink(fullTargetPath, this.id);
                requestContext.getRegistry().removeLink(this.mountPoint);
                if (/*registryContext != null*/ registryService != null) {
//                    HandlerManager hm = registryContext.getHandlerManager();
    ...
    public void setConURL(String conURL) {
        if (conURL.endsWith(RepositoryConstants.PATH_SEPARATOR)) {
            this.conURL = conURL.substring(0, conURL.lastIndexOf(
                    RepositoryConstants.PATH_SEPARATOR) - 1);
        }
        this.conURL = conURL;
    }
        ...
        if (subPath.equals(RepositoryConstants.ROOT_PATH)) {
            ...
            this.subPath = "";
                ...
                subPath.charAt(subPath.length() - 1) ==
                        RepositoryConstants.PATH_SEPARATOR.charAt(0)) {
