...
//                DEFAULT_SYSTEM_HANDLER_PHASE).getRegistryContext(requestContext);
        RegistryContext defaultValue = getRegistryContext(handlerManagers.get(
                DEFAULT_SYSTEM_HANDLER_PHASE), requestContext);
        boolean isProcessingComplete = requestContext.isProcessingComplete();
        if (!isProcessingComplete) {
...
//                    TENANT_SPECIFIC_SYSTEM_HANDLER_PHASE).getRegistryContext(requestContext);
            RegistryContext tenantSpecificValue = getRegistryContext(handlerManagers.get(
                    TENANT_SPECIFIC_SYSTEM_HANDLER_PHASE), requestContext);
            if (tenantSpecificValue != null) {
                defaultValue = tenantSpecificValue;
            }
            isProcessingComplete = requestContext.isProcessingComplete();
...
//                    USER_DEFINED_SYSTEM_HANDLER_PHASE).getRegistryContext(requestContext);
            RegistryContext systemSpecificValue = getRegistryContext(handlerManagers.get(
                    USER_DEFINED_SYSTEM_HANDLER_PHASE), requestContext);
            if (systemSpecificValue != null) {
                defaultValue = systemSpecificValue;
            }
            isProcessingComplete = requestContext.isProcessingComplete();
...
//                USER_DEFINED_HANDLER_PHASE).getRegistryContext(requestContext);
        RegistryContext userDefinedValue = getRegistryContext(handlerManagers.get(
                USER_DEFINED_HANDLER_PHASE), requestContext);
        isProcessingComplete |= requestContext.isProcessingComplete();
        // The reporting handler phase needs to know about the state of processing
        requestContext.setProcessingComplete(isProcessingComplete);
//        handlerManagers.get(DEFAULT_REPORTING_HANDLER_PHASE).getRegistryContext(requestContext);
     ...
     */
    public RegistryContext getRegistryContext(HandlerManager handlerManager, RequestContext requestContext) {
...
//                    registryContext = handler.getRegistryContext(requestContext);
                	if(handler instanceof MountHandler) {
                		registryContext = ((MountHandler) handler).getRegistryContext(requestContext);	// This is overidden only in MountHandler
                	}
